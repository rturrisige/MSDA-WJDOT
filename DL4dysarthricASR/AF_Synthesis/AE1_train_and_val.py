...
import DNN_Input  # DNN_Input.py if the loader 
import os
...
from AE1_config import AE1_train_and_val_filenames
...
# -------------------------------Network parameters-------------------------------
motor_lambda = 2  # penalization term of the motor loss function
biases_init= 0.1
batch_size = 500
...
filenames_train, validation_audio = AE1_train_and_val_filenames(data_dir, WindowSize, Number_of_TrainFiles)
...
# (Speaker normalized) audio data with context
...
trainingLogFile.write('motor lambda = ' + str(motor_lambda) + '\n')
...
print('motor lambda = ' + str(motor_lambda) + '\n')
print('\n')
...
def train(filenames_train=filenames_train, audio_val=audio_val):
    ...
    with tf.Graph().as_default():
        with tf.device('/cpu:0'):
            with tf.variable_scope('input'):
                audio_batch_val = tf.placeholder("float", [None, n_features])
            print('Placeholder initialized.')

            with tf.variable_scope('training_batch'):
                audio_batch_train, motor_batch_train = \
        ...
        with tf.device('/gpu:1'):
            ...
            with tf.variable_scope('training_graph') as train_scope:
                reconstructed_audio_train, motor_prediction_train = model(audio_batch_train, n_features, n_hidden_1, n_hidden_2,
          n_hidden_3, n_hidden_4, biases_init)
                with tf.variable_scope('cost'):
                    ...
                    cost = audio_cost + motor_lambda * motor_cost
            ...
            with tf.name_scope('training_evaluation'):
                with tf.variable_scope(train_scope, reuse=True):
                    reconstructed_audio_val, motor_prediction_val = model(audio_batch_val, n_features, n_hidden_1, n_hidden_2,
          n_hidden_3, n_hidden_4, biases_init)
                ...
                while not coord.should_stop():
                    ...
                    if (step % int(num_examples_per_epoch / batch_size) == 0) and (step is not 1):
                        ...
                        if (epoch_counter % 1) == 0:
                            ...
                            audio_corr_val = 0.0
                            audio_err_val = 0.0
                            for i in range(0, audio_val.shape[0]-1, step_val):
                                audio_prediction_i = sess.run(reconstructed_audio_val, feed_dict={audio_batch_val: audio_val[i:i + step_val, :]})                              
                            ...
                            # Early stopping is only based on the audio part.
                            ...
                            if counter4earlystopping > 4:
                               print('\nDone training (Early stopping)')
                               end_time = datetime.now()
